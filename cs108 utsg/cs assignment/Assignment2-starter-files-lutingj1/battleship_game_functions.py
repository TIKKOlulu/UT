...
def valid_cell_indexes(row: int, col: int, grid_size: int) -> bool:
    ...
    if row not in list(range(grid_size)):
        return False
    if col not in list(range(grid_size)):
        return False
    else:
        return True
...
def is_win(ship_sizes: list[int], hits_list: list[int]) -> bool:
    ...
    for i in range(0, len(ship_sizes)):
...
def update_fleet_grid(row: int, col: int, fleet_grid: list[list[str]],
    ...
    for i in range(len(ship_symbols)):
        if fleet_grid[row][col] == ship_symbols[i]:
            hits_list[i] += 1
...
def has_ship(fleet_grid: list[list[str]], row_start: int, col_start: int,
    ...
    if fleet_grid[r][c] != ship_symbol:
        return False

    while i + c < len(fleet_grid) and fleet_grid[r][c + i] == ship_symbol:
        i += 1
        row += 1

    while j + r < len(fleet_grid) and fleet_grid[r + j][c] == ship_symbol:
        j += 1
        col += 1
        
    if ship_size == row == col == 1:
        return True
    elif row == 1 and col == ship_size:
        return True
    else:
        return False
...
def validate_symbol_counts(fleet_grid: list[list[str]],
    ...
    for row in fleet_grid:
        for item in row:
            if item == EMPTY:
                empty += 1
            elif item in ship_symbols:
                i = ship_symbols.index(item)
                size[i] -= 1
    for s in size:
        if s != 0:
            return False
    if empty + sum(ship_sizes) != len(fleet_grid) * len(fleet_grid):
        return False
    else:
        return True
...
def validate_ship_positions(fleet_grid: list[list[str]],
    ...
    for item in new:
        for i in range(len(ship_symbols)):
            symbol = ship_symbols[i] * ship_sizes[i]
            if symbol in item:
                count += 1
    if count == len(ship_symbols):
        return True
    return False
